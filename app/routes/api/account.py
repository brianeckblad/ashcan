...
def setup_totp() -> Response:
    ...
    The secret is stored only as short-lived setup state until confirmed via
    ...
    try:
        ...
        session['totp_setup_created'] = time.time()
...
def enable_totp() -> Response:
    ...
    Request Body (JSON)::
        ...
        {"password": "<current-password>", "code": "<6-digit>"}

    The ``code`` must be valid for the short-lived setup secret generated by
    ``/api/account/totp/setup``. Current password is required so a stolen
    session alone cannot enroll an attacker-controlled authenticator.
    ...
    try:
        ...
        setup_created = session.get('totp_setup_created', 0)

        if not password or not code:
            return jsonify({'success': False, 'error': 'Password and verification code are required'}), 400

        if not user_manager.verify_password(username, password):
            return jsonify({'success': False, 'error': 'Password is incorrect'}), 400

        try:
            setup_age = time.time() - float(setup_created or 0)
        except (TypeError, ValueError):
            setup_age = 601

        if not secret or setup_age > 600:
            session.pop('totp_setup_secret', None)
            session.pop('totp_setup_created', None)
            return jsonify({'success': False, 'error': 'Setup expired. Generate a new QR code and try again.'}), 400
